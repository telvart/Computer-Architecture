...
caller:		addi 	$a0, $zero, 15		# initialize argument register $a0 with n = 15
		#addi	$a1, $zero, 0x10010000	# intialize argument register $a1 with the starting address
		lui	$a1, 0x1001		# intialize argument register $a1 with upper part of the starting address
		ori	$a1, 0x0000		# intialize argument register $a1 with lower part of the starting address
		jal	fib_seq			# call fib_seq
next:		j	exit

##############################################################################
# This routine generates Fibonacci sequence fib(0), fib(1), fib(2), ... fib(n)
# and stores it into the data segment starting at memory address stored in $a1
